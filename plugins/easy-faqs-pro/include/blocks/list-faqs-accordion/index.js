( 
	...
	var get_theme_group_label = function(theme_group_key) {
		if ( typeof(easy_faqs_admin_list_faqs_accordion.theme_group_labels[theme_group_key]) !== 'undefined' ) {
			return easy_faqs_admin_list_faqs_accordion.theme_group_labels[theme_group_key];
		}
		return 'Themes';
	};	
	...
	var get_theme_options = function() {
		var theme_opts = [];
		for( theme_group in easy_faqs_admin_list_faqs_accordion.themes ) {
			//theme_group_label = get_theme_group_label(theme_group);
			for ( theme_name in easy_faqs_admin_list_faqs_accordion.themes[theme_group] ) {
				theme_opts.push({
					label: easy_faqs_admin_list_faqs_accordion.themes[theme_group][theme_name],
					value: theme_name,
				});				
			}
		}
	 ...
	 */
	registerBlockType( 'easy-faqs/list-faqs-accordion', {
		/**
		...
		edit: wp.data.withSelect( function( select ) {
					return {
						...
						if ( !easy_faqs_admin_list_faqs_accordion.is_pro ) {
							theme_fields.push(
								el(  
									'a',
									{ 
						...
						var inner_fields = [];
						inner_fields.push( el('h3', { className: 'block-heading' }, 'Easy FAQs - FAQs Accordion') );
						inner_fields.push( el('blockquote', { className: 'faq-list-placeholder' }, __('A collapsable list of your FAQs from your database.') ) );
						retval.push( el('div', {'className': 'easy-faqs-editor-not-selected'}, inner_fields ) );

				return el( 'div', { className: 'easy-faqs-list-faqs-accordion-accordion-editor'}, retval );
